...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("status", postgresql.ENUM(name="referral_status_enum", create_type=False), nullable=False),
    ...
    # --- 5. Create Task Enums & Tasks Table ---
    ...
    op.create_table(
        ...
        sa.Column("job_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("jobs.id", ondelete="CASCADE"), nullable=False),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False),
        sa.Column("task_type", postgresql.ENUM(name="task_type_enum", create_type=False), nullable=False),
